...
import { stringWidth, unicodeWidth } from '../mod.ts'
...
Deno.test('property-testing equality with unicode-width Rust crate', async (t) => {
	...
	try {
		try {
			// @ts-ignore type-check errors if unavailable due to lack of --unstable flag
			dylib = Deno.dlopen(libPath, { unicode_width: { parameters: ['buffer'], result: 'usize' } })
		} catch {
			ignore = true
		}
			...
			await t.step({
				...
				fn() {
					fc.assert(
						fc.property(
							fc[arbitrary](),
							// JSON stringify to allow null chars to cross FFI boundary in a null-terminated string
